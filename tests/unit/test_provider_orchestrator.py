...
def test_orchestrator_runs_baseline_provider(tmp_path: Path) -> None:
...
def test_orchestrator_returns_controlled_error_for_selected_unregistered_adapter(tmp_path: Path) -> None:
    ...
    with pytest.raises(ProviderError) as exc_info:
        analyze_with_provider_selection(
            track_id="track-1",
            path=audio_path,
            requested_provider="librosa",
            safe_baseline="baseline",
            provider_names=["librosa"],
        )

    assert exc_info.value.details.code == "provider_unavailable"
    assert exc_info.value.details.provider == "librosa"
